...
const docModules = import.meta.glob<string>("/src/routes/blog/**/*.svx", {
  query: "?raw",
  eager: true,
  import: "default",
});

const metaModules = import.meta.glob<DocModule>("/src/routes/blog/**/*.svx", {
  eager: true,
});
...
function parseDocs() {
  ...
  for (const path in docModules) {
    ...
    const cleanPath = path.replace("/src/routes/blog/", "").replace("/+page.svx", "");
    const slug = `/blog/${cleanPath}`;
...
export function searchDocs(query: string): DocSection[] {
  ...
  const groups = new Map<string, { parent: DocSection; children: DocSection[]; maxScore: number }>();
