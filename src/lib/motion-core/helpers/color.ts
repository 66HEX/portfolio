...
export const parseCssColor = (value: string): [number, number, number] | null => {
  ...
  cssColorContext.fillStyle = "#ffffff";
  cssColorContext.fillStyle = value;
  if (cssColorContext.fillStyle !== normalized) return null;
  ...
  cssColorContext.fillStyle = normalized;
  cssColorContext.fillRect(0, 0, 1, 1);
  const [r, g, b, a] = cssColorContext.getImageData(0, 0, 1, 1).data;
  return a === 0 ? null : normalizeTriplet(r, g, b);
};

export const toRgb = (value: ColorRepresentation, fallback: [number, number, number]): [number, number, number] => {
