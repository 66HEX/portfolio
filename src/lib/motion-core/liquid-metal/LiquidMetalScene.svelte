<script lang="ts">
  ...
  const applyColor = (target: Vec3, value: ColorRepresentation, fallback: [number, number, number]) => {
  ...
  const mountScene: Attachment<HTMLCanvasElement> = (targetCanvas) => {
    ...
    const localUniforms = {
      ...
      uBackgroundColor: { value: new Vec3(23 / 255, 24 / 255, 26 / 255) },
      uSpeed: { value: 1 },
      uScale: { value: 1 },
      uFrequency: { value: 3.5 },
      uRefraction: { value: 2 },
      uChromaticAberration: { value: 3 },
      uBlur: { value: 1 },
    };
    ...
    $effect(() => {
      ...
      localUniforms.uFrequency.value = frequency;
